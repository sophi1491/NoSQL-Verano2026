...
6. Mostrar los usuarios que estudien Ingeniería Informática y estén en sexto semestre o 
superior.
...
db["libros"].find({año:{$gt:2022}},{_id: 0, titulo:1 ,año:1}).
  sort({año:-1})
...
db["usuarios"].find({
  carrera:{$in:['Ingeniería en Sistemas Computacionales','Ingeniería Informática']}},
                    {_id:0,nombre:1,carrera:1})
...
20. Mostrar los préstamos no devueltos, ordenados por la cantidad de días de préstamo de mayor a menor, mostrando únicamente el usuario, el libro y los días de préstamo.
  db["prestamos"].find({devuelto:false},{usuario:1,libro:1,_id:0,diasPrestamo:1}).sort({diasPrestamo: -1})
